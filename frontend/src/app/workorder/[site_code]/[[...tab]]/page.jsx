...
export default function WorkOrderPage() {
  ...
  // Tab path parsing from catch-all [[...tab]]
  // tab param can be undefined, ['master'], ['battery'], ['battery', '1', '1'], etc.
  const tabArray = Array.isArray(params?.tab) ? params.tab : (params?.tab ? [params.tab] : []);
  const activeTab = tabArray[0] || 'master';
  ...
  useEffect(() => {
    ...
    fetch(`/api/sites`)
      .then(res => res.json())
      .then(data => {
        const dbSite = Array.isArray(data) ? data.find(s => s.site_code === site_code) : null;
        if (dbSite) {
        ...
        <div className="border-b border-dark-border">
          <nav className="flex flex-wrap gap-2 -mb-px">
            {tabList.map((tabItem) => (
              <button
                key={tabItem.id}
                onClick={() => router.push(`/workorder/${site_code}/${tabItem.id}`)}
